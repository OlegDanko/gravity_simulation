...
void GLSimShaders::calc_forces(const std::vector<std::pair<size_t, size_t> > &elements_pairs,
...
void GLSimShaders::calc_velocities(const std::vector<size_t> &elements_ids,
                                   const mass_tex_t &mass_a_in,
                                   const mass_tex_t &mass_b_in,
...
void GLSimShaders::update_pos_vel(const std::vector<size_t> &elements_ids,
                                  const delta_v_matrix_t &delta_v_in,
                                  size_t chunks_count, size_t chunk_id,
...
std::string GLShaders::calc_forces_vertex = R"(
#version 330 core

layout (location = 0) in ivec2 elements_pair;

uniform sampler1D pos_a_in;
uniform sampler1D pos_b_in;
uniform sampler1D mass_a_in,
uniform sampler1D mass_b_in,
uniform float pixel_size;
uniform float pixel_offset;

uniform int forces_out_size,
out vec3 force_out;
...
void main()
{
    ...
    forces_out.at(a).at(b) = force_vec;
    gl_Position = vec4(pixel_offset + a_pos*pixel_size,
                       pixel_offset + b_pos*pixel_size,
                       0.0, 1.0);
}
)";
std::string GLShaders::calc_velocities_vertex = R"(
#version 330 core
...
std::string GLShaders::update_pos_vel_vertex = R"(
#version 330 core
